...
data <- data.frame(x=1:18, y=c(3,2,2.2,6,5,1.8,3,6.5,8,7,8.5,10,8,7.7,6.5,7,6,7.5))
# line <- data.frame(x=1:18,y=0)

p <- ggplot(aes(x = x, y = y), data = data) +
  geom_line(color = "white", size = 1.3) +
  geom_point(size = 3.5, color = "#FFFFFF", data = data[c(5,13),])+
  geom_point(size = 3.3, color = "#18BC9C", data = data[c(5,13),])+
